PATH = '.\\texts\\tracking.txt'
blacklist = ['Dont', 'A', 'A Few', 'A Little', 'All', 'An', 'Another', 'Any', 'Anybody', 
             ...
             'At', 'So', 'With', 'Like', 'Your', 'Can', 'But', 'When', 'Its', 'Are', 'If', 'It\'s', 'Don\'t',
             'I\'m', '-', 'As', 'Get', 'Or']
def update(person, string):
    ...
    for word in string:
        ...
        if word[-1] in '.,!?':
            word = word[:-1]
        if word in blacklist or '<' in word or word.isnumeric():
            continue
...
if text:
    word_dict = eval(text)
for person in word_dict:
    temp = copy.deepcopy(word_dict[person])
    for name in temp:
        if '<' in name:
            word_dict[person].pop(name, None)
with open(PATH, 'w', encoding='utf-8') as f:
